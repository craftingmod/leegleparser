...
export class Logger{
  ...
  private static getLocalISODate():string{
    ...
    const offsetStamp = new Date(now.getTime())
    const pad = (n:number, ln:number) => n.toString(10).padStart(ln, "0")
    const ampm = offsetStamp.getHours() >= 12 ? "P" : "A"
    let hour12 = (offsetStamp.getHours() % 12)
    if (hour12 == 0) {
      hour12 = 12
    }
    return `${ampm}_${pad(hour12,2)}:${pad(offsetStamp.getMinutes(),2)}:${pad(offsetStamp.getSeconds(), 2)}.${pad(offsetStamp.getMilliseconds(), 3)}_${offset && toSignedString(offset)}`
  ...
  private static get directoryPath():string{
    return `${__dirname}/${settings.directory}/${Logger.subject}`;
  }
